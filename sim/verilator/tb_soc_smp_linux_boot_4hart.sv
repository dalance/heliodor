// SystemVerilog wrapper to run the N=4 SMP SoC Linux boot on an external SV
// simulator. Drives the shared SMP harness with N_HARTS=4 + the 4-hart hex
...
module tb_soc_smp_linux_boot_4hart;
    logic        clk;
    logic        rst;
    logic        pass;
    logic [63:0] r3;
    ...
    initial begin
        ...
        for (longint unsigned i = 0; i < MAX_CY; i++) begin
            ...
            if (pass) break;
            if (i % 5_000_000 == 0)
                $display("  [verilator N=4] cy=%0d r3=%h", cy, r3);
        end
        $display("[tb_soc_smp_linux_boot_4hart] pass=%0b r3=%h cy=%0d", pass, r3, cy);
        if (pass)
            $display("SMP(N=4) LINUX BOOT PASSED on Verilator (SBI shutdown, x3==0xAA)");
        else
            $display("SMP(N=4) did NOT reach SBI shutdown within %0d cy", MAX_CY);
        $finish;
    end
endmodule
